...
6. **Создание снапшотов**:
   ...
   - Проверяем состояние снапшота через `CheckSnapshotStateInRepo` (использует `GetSnapshotsIgnore404` для корректной обработки отсутствующих snapshots):
   ...
   - Если снапшота нет или он не в состоянии `SUCCESS`:
     ...
     - Создаем снапшот через `CreateSnapshotWithRetry` с retry логикой (до 7 попыток)
   - **Retry логика в `CreateSnapshotWithRetry`**:
     - После создания snapshot начинается polling цикл для проверки статуса
     - Таймаут ожидания появления snapshot: 15 минут с момента создания
     - **Обработка ошибок при polling**: Для любых ошибок (включая 404) ждем до таймаута (15 минут), затем переходим к следующей попытке retry (если попытки остались) или возвращаем ошибку
     - **Обработка PARTIAL/FAILED**: При обнаружении PARTIAL/FAILED snapshot удаляем его, ждем 15 минут, затем переходим к следующей попытке retry - были случаи когда нужно просто подождать
     - **Обработка неизвестных состояний**: При неизвестном состоянии snapshot переходим к следующей попытке retry после короткой паузы
     - Все переходы к следующей попытке используют метку `retryLoop` для явного управления потоком выполнения
   - **Алертинг**: При неудаче после всех попыток отправляется алерт в Madison через `SendMadisonSnapshotCreationFailedAlert` (логируется попытка отправки и результат)
...
7. **Создание снапшота**:
   - Получаем существующие снапшоты через `GET /_snapshot/{repo}/*{today}*` с обработкой 404 через `GetSnapshotsIgnore404`
   - Проверяем/удаляем ошибочный снапшот через `CheckAndCleanSnapshot`
   - Создаем снапшот через `CreateSnapshotWithRetry` с retry логикой (до 7 попыток, таймаут 15 минут, обработка всех ошибок единообразно)
...
**Алгоритм:**
1. **Расчёт утилизации**: Получаем `GET /_cat/nodes?h=dup&format=json`, считаем среднюю утилизацию по всем нодам (поле `dup` содержит disk usage percent)
2. **Если утилизация <= порога**: Завершаем выполнение
3. **Валидация параметров**: Проверяется, что `retention_days_count >= 2`. Если значение меньше 2, команда завершается с ошибкой.
4. **Вычисление cutoff date**: На основе параметра `retention_days_count` (минимум 2 дня) вычисляется дата отсечки. Индексы новее этой даты не будут удаляться.
...
7. **Dry run режим**: Показываем первые 5 индексов для удаления с их размерами
8. **Удаление индексов**:
   - Для каждого индекса из списка для удаления:
     ...
     - Пересчитываем утилизацию через `GET /_cat/nodes?h=dup&format=json`
     - **Останавливаем удаление**, если утилизация стала меньше или равна порогу
9. **Summary**: В конце выводится summary с финальной утилизацией, списком успешно удаленных индексов и списком неудачных удалений
...
5. **Dry run режим**:
   ...
   - Показываем план создания снапшотов с указанием репозитория для каждой группы
6. **Создание снапшотов**:
   - Для каждой группы проверяем существующие снапшоты через `GET /_snapshot/{repo}/*{date}*` с обработкой 404 через `GetSnapshotsIgnore404`
   - Проверяем состояние снапшота через `CheckSnapshotStateInRepo` (использует `GetSnapshotsIgnore404` для корректной обработки отсутствующих snapshots):
     - Если снапшот уже в состоянии `SUCCESS`:
       - Проверяем наличие всех индексов из группы в существующем снапшоте
       - Если все индексы присутствуют - пропускаем с логированием
   ...
   - **Retry логика в `CreateSnapshotWithRetry`**:
     ...
     - Таймаут ожидания появления snapshot: 15 минут с момента создания
     - **Обработка ошибок при polling**: Для любых ошибок (включая 404) ждем до таймаута (15 минут), затем переходим к следующей попытке retry (если попытки остались) или возвращаем ошибку
     - **Обработка PARTIAL/FAILED**: При обнаружении PARTIAL/FAILED snapshot удаляем его, ждем 15 минут, затем переходим к следующей попытке retry
     - **Обработка неизвестных состояний**: При неизвестном состоянии snapshot переходим к следующей попытке retry после короткой паузы
     - Все переходы к следующей попытке используют метку `retryLoop` для явного управления потоком выполнения
