...
5. **Получение кандидатов**:
   ...
   - Фильтруем индексы используя `utils.ShouldSkipIndex` (исключаем системные индексы, restored-, extracted_) - если наша цель освободить диски, то restored/extracted надо резать первыми
...
8. **Удаление индексов**:
   - Для каждого индекса в порядке убывания размера:
     ...
     - Если `retention_check_snapshots=true`: Проверяем наличие валидного снапшота через `HasValidSnapshot`
   ...
   - **Удаление с проверкой порога**:
     - Для каждого индекса, прошедшего проверку (или без проверки, если `retention_check_snapshots=false`):
       - Удаляем индекс через `DELETE /{index}`
       - После каждого удаления делаем паузу 15 секунд - зачем?
...
- Индексы фильтруются функцией `utils.ShouldSkipIndex` (как в `snapshotsbackfill`) для исключения системных индексов, restored- и extracted_ индексов.
...
4. **Группировка для снапшотов**: Группируем индексы через `GroupIndicesForSnapshots` - где можно посмотреть? что такое "группы" тут?
5. **Dry run режим**:
...
6. **Создание снапшотов**:
   ...
   - Если снапшота нет или он не в состоянии `SUCCESS`:
     - Для каждой группы проверяем/удаляем ошибочный снапшот через `CheckAndCleanSnapshot`
     - Создаем снапшот через `CreateSnapshotWithRetry` с retry логикой
   - **Пауза**: После каждого создания снапшота делаем паузу 10 минут перед следующим - зачем?
...
12. **Создание нового шаблона**:
    ...
    - Если индекс почасовой (имеет суффикс после даты) - добавляем `composed_of: ["default_template"]` - почему именно так? что в default_template?
...
2. **Нормализация URL**: Добавляем схему `https://` к `osd-url` если отсутствует через `NormalizeURL` - это только тут? или работает с os-url тоже?
