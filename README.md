...
│   │   ├── indicesdelete.go     # Удаление индексов согласно values ( с проверкой из либы snapshotchecker, с генерацией алерт  - не могу удалить индекс, с флагом нужно ли это вообще)
...
3. **Удаление снапшотов**: делаем по алогритму - выносим
...
4. **Удаление индексов**: делаем по алогритму  - выносим
...
curator-go snapshot --index=infra --snap_repo=s3-backup

# Снапшот unknown индексов
curator-go snapshot --unknown --snap_repo=s3-backup

# Только удаление
curator-go snapshot-delete --index=infra --snap_repo=s3-backup
```
...
2. **Если утилизация > порога**:
   ...
   - Для каждого индекса:
     - Проверяем snapshots в новом репо через `GET /_snapshot/{snap_repo}/*-{tomorrow}`
     - Если оба есть - удаляем индекс через `DELETE /{index}`
...
**Флаги:**
```bash
curator-go retention --threshold=75 --snap_repo=s3-backup --endpoint=opendistro
```
...
```bash
# Обычные индексы
curator-go sharding --shard-size=26843545600
...
```bash
# Без multitenancy
curator-go indexpatterns --multitenancy=false --regex="^(.*?)-\d{4}\.\d{2}\.\d{2}.*$" --kibana-host=https://kibana.example.com

# С multitenancy
curator-go indexpatterns --multitenancy=true --tenants="tenant1,tenant2" --kibana-host=https://kibana.example.com

# С recoverer
curator-go indexpatterns --multitenancy=false --regex="^(.*?)-\d{4}\.\d{2}\.\d{2}.*$" --recoverer-enabled=true --kibana-host=https://kibana.example.com
```
...
**Флаги:**
```bash
curator-go datasource --title=recoverer --os-url=https://opendistro-recoverer:9200 --kibana-host=https://kibana.example.com

# С multidomain
curator-go datasource --title=recoverer --os-url=https://opendistro-recoverer:9200 --kibana-host=https://kibana.example.com --multidomain=true --namespace=default
```
...
curator-go dereplicator --days-count=2 --use-snapshot=true --snap_repo=s3-backup --os-url=https://opendistro:9200
```
...
```bash
# Whitelist режим
curator-go snapshotchecker --snap_repo=s3-backup --whitelist="nginx,curator-go" --madison-key=xxx --madison-project=yyy --kibana-host=https://kibana.example.com (для алертинга только?)

# Exclude режим
curator-go snapshotchecker --snap_repo=s3-backup --exclude-list="system" --madison-key=xxx --madison-project=yyy --kibana-host=https://kibana.example.com 

# Без Madison
curator-go snapshotchecker --snap_repo=s3-backup --whitelist="nginx,curator-go"
```
...
**Флаги:**
```bash
curator-go danglingchecker --os-url=https://opendistro:9200 --kibana-host=https://kibana.example.com --madison-key=xxx --madison-project=yyy

# Без Madison
curator-go danglingchecker --os-url=https://opendistro:9200 --kibana-host=https://kibana.example.com
```
...
**Флаги:**
```bash
curator-go coldstorage --index-pattern="*" --set-replicas-zero=true --move-to-cold=true --os-url=https://opendistro:9200
```
...
```bash
# Обычные сертификаты
curator-go extracteddelete --selector="extracted_*" --os-url=https://opendistro:9200

# С recoverer сертификатами
curator-go extracteddelete --selector="extracted_*" --os-url=https://opendistro-recoverer:9200 --use-recoverer-certs=true
```
...
(поправить везде чтобы было - в флагах )